...
3. **Set Fetch Depth**: Always use `fetch-depth: 0` in checkout for accurate diffs
4. **Keep Patterns Simple**: Use broad include patterns and specific exclude patterns
5. **Use Config Files**: For complex configurations, use external YAML files
6. **Test Configuration**: Use `strict_validation: true` during setup to catch issues early
7. **Document Patterns**: Add comments to configuration files explaining each pattern's purpose

## Performance Considerations

- Optimized for monorepos with hundreds of files
- Pattern matching uses efficient glob libraries
- Git operations delegated to GitHub's API
